...
import com.lianjia.matrix.common.command.registry.support.DefaultAuthHandler;
 ...
 * @Created
 */
public abstract class AbstractInstructionSender implements InstructionSender, InstructionRecieveableCommandRegistry, Lifecycleable, InstructionListener, ProjectStatusListener, ProjectStatusChangeWatchable {

    protected List<InstructionListener> instructionRecieveListeners = new ArrayList<>();

    protected List<ProjectStatusListener> projectStatusListeners = new ArrayList<>();

    private AuthHandler authHandler;

    protected Project selfProject;

    public AbstractInstructionSender() {
    ...
    public void onProjectRegisted(Project project) {
        for (ProjectStatusListener listener : projectStatusListeners) {
            listener.onProjectRegisted(project);
        }
    }
    ...
    public void onProjectDetached(Project project) {
        for (ProjectStatusListener listener : projectStatusListeners) {
            listener.onProjectDetached(project);
        }
    }
    ...
    @Override
    public void addProjectStatusChangeListener(ProjectStatusListener listener) {
